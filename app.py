...
def start_kyc_route():
    ...
    with lock:
        if not kyc_started:
            ...
            possible_actions = ["smile", "turn left", "turn right", "look up", "look down", "blink", "type code"]#"follow dot"
            # Ensure 'blink' and 'type code' are included
            selected_actions = random.sample([action for action in possible_actions if action not in ["type code"]], 3) + ["type code"]
